...
npx skills add github.com/Taste-AI/skills/tree/main/skills/taste-director
...
| [`taste-director`](skills/taste-director/SKILL.md) | direct a whole page or brand system from evidence: search with `brand-search`, lock the ranked references as authority, record every value with its source in `BRAND.json`, build only from that file, then verify the result beside its references. Requires `brand-search` |
...
`get_brand` / `find_similar_brands`). `taste-director` runs on top of `brand-search` and adds `lookup_slop` to
