...
The decisions document is a **Taste Engine extraction response populated by
the agent**: write the `design_system` object that `get_brand_extraction_result`
WOULD return if
...
- Color values: OKLCH is the source of truth. `colors` carries the primitives
  and semantic tokens the page will declare, each with a name, its OKLCH
  value, and a `rules.description` saying where it may be used. Preserve the
  source's uppercase 6-digit hex in the nearest `notes` or `description` field
  for provenance and conversion checks. Every color in the CSS resolves to a
  named OKLCH token. Transparency ships as an explicit overlay token with its
  alpha recorded in the description.
...
exist in the real schema and verify walks them, so fill them in this
shape:
...
budgets; verify counts them.

## Provenance

Every taste value, a face, an OKLCH color with its source hex, an easing, a
radius, a texture classification, a section anatomy, names the source that taught it, in
...
citation is a facet nobody decided; verify treats it as undecided, and
